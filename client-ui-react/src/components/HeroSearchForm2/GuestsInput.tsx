...
const GuestsInput: FC<GuestsInputProps> = ({
    ...
    return (
        <div className={`flex relative ${className}`} ref={refContainer}>
            ...
            {isOpen && (
                <div
                    className="absolute right-0 z-30 w-full sm:min-w-[340px] max-w-sm bg-white dark:bg-neutral-800 top-full mt-3 py-5 sm:py-6 px-4 sm:px-8 rounded-3xl shadow-xl">
                    <NcInputNumber
                        className="w-full"
                        defaultValue={guestAdultsInputValue}
                        onChange={(value: any) => handleChangeData(value, "guestAdults")}
                        max={10}
                        min={1}
                        label="Adults"
