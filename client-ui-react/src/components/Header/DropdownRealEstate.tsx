...
export default function DropdownRealEstate() {
    const location = useLocation<any>();
    const [activeName, setActiveName] = useState<string>('');
    const {t} = useTranslation();

    useEffect(() => {
        let blogPath = ['add-blog', 'blog']
        if (blogPath.includes(location.pathname.toString().split('/')[1])) {
            setActiveName('Blog')
        } else {
            setActiveName(t('Real Estate'))
        }
    }, [location])
        ...
        <div className="DropdownTravelers">
            <Popover className="relative">
                {({open, close}) => (
                    <>
                        <Popover.Button
                            className={`${open ? "" : "text-opacity-90"} group py-2 rounded-md text-sm sm:text-base font-medium hover:text-opacity-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75`}
                        >
                            <div
                            ...
                            >
                                ...
                                <ChevronDownIcon
                                    className={`${open ? "-rotate-180" : "text-opacity-70 "}ml-2 h-5 w-5 text-neutral-700 group-hover:text-opacity-80 transition ease-in-out duration-150 `}
                                    aria-hidden="true"
                                />
                            </div>
                        ...
                        >
                            <Popover.Panel
                                className="absolute z-40 w-screen max-w-xs px-4 mt-3 transform -translate-x-1/2 left-1/2 sm:px-0">
