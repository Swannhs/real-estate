...
const Card12: FC<Card12Props> = ({
    ...
    return (
        ...
        >
            <Link
                to={import.meta.env.VITE_APP_BLOG_SINGLE_PREFIX + `/${post?.subUrl}`}
                className="block flex-shrink-0 flex-grow relative w-full h-0 aspect-w-4 aspect-h-3 rounded-3xl overflow-hidden"
            >
                <NcImage
                    containerClassName="absolute inset-0"
                    src={import.meta.env.VITE_APP_BLOG_PUBLIC_URL as string + post?.blogCoverImage}
                    alt={post?.title}
                />
                <span>
            ...
            <div className=" mt-8 pr-10 flex flex-col">
                ...
                >
                    <Link to={import.meta.env.VITE_APP_BLOG_SINGLE_PREFIX + `/${post?.subUrl}`} className="line-clamp-2"
                          title={post?.title}>
                        {post?.title}
                    </Link>
                </h2>
                <span className="hidden sm:block mt-4 text-neutral-500 dark:text-neutral-400">
                    <span className="line-clamp-2"> {post?.shortDescription}</span>
                </span>
