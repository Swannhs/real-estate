...
function ReactTable({ columns, data, top }: { columns: Column[]; data: []; top?: boolean }) {
  ...
  return (
    <Stack>
      {top && (
        <Box sx={{ p: 2 }}>
          <TablePagination gotoPage={gotoPage} rows={rows} setPageSize={setPageSize} pageIndex={pageIndex}
                           pageSize={pageSize} />
        </Box>
      )}
    ...
    <Grid container spacing={3}>
      <Grid item xs={12}>
        <MainCard title='Pagination at Top' content={false}
                  secondary={<CSVExport data={data} filename={'pagination-top-table.csv'} />}>
          <ScrollX>
