...
const ExpandingDetails = ({ data }: { data: [] }) => {
  ...
  const renderRowSubComponent = useCallback(({ row: { id } }: { row: Row<{}> }) => <ExpandingUserDetail
    data={data[Number(id)]} />, [data]);

  return (
    <MainCard title='Expanding User Details' content={false}
              secondary={<CSVExport data={data} filename={'expanding-details-table.csv'} />}>
      <ScrollX>
